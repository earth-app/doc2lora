...
@cli.command()
@click.argument('documents_path', type=click.Path(exists=True, file_okay=False, dir_okay=True))
@click.option('--output', '-o', default='lora_adapter.json', help='Output path for the LoRA adapter')
@click.option('--model', '-m', default='microsoft/DialoGPT-small', help='Base model name for fine-tuning')
@click.option('--max-length', default=512, help='Maximum sequence length for tokenization')
